...
const windowListener = (name: string) =>
  (dispatch: any, action: any): Function => {
...
// }
export const handleValueWith = (f: Function) =>
  <S>(state: State<S>, event: any): any => {
    return f (state, event.target.value)
  }

// Invokes a collection of event handlers for the same event.
export const handleUsing = (handlers: Function[]) =>
  <S, E>(state: State<S>, event: E): any => {
